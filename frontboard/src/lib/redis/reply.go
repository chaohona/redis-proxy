...
var ErrNet = errors.New("[redis] connect error")
...
func Float64s(reply interface{}, err error) ([]float64, error) {
	...
	switch reply := reply.(type) {
	case []interface{}:
		result := make([]float64, len(reply))
		for i := range reply {
			if reply[i] == nil {
				continue
			}
			switch rp := reply[i].(type) {
			case float64:
				if rp < 0 {
				...
				result[i] = float64(rp)
			case []byte:
				n, err := strconv.ParseFloat(string(rp), 64)
				if err != nil {
					return nil, err
				}
				result[i] = n
...
func BytesArray(reply interface{}, err error) ([][]byte, error) {
	if err != nil {
		return nil, err
	}
	switch reply := reply.(type) {
	...
	case []interface{}:
		...
		for i := range reply {
			...
			case int64:
				result[i] = int64(rp)
			case []byte:
				n, err := strconv.ParseInt(string(rp), 10, 64)
				if err != nil {
					return nil, err
				}
				result[i] = n
	...
	case []interface{}:
		...
		for i := range reply {
			...
			case int64:
				...
				result[i] = uint64(rp)
			case []byte:
				n, err := strconv.ParseUint(string(rp), 10, 64)
				if err != nil {
					return nil, err
				}
				result[i] = n
